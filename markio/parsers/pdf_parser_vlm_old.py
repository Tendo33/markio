...
def _vlm_doc_analyze_wrapper(pdf_bytes, image_writer, backend, server_url):
    """VLM document analysis wrapper function, ensures model is initialized"""
    try:
        ...
        logger.error(f"Error in VLM analysis wrapper: {e}")
        raise
...
async def pdf_parse_vlm_main(
    ...
    try:
        # Run VLM analysis in a thread pool to avoid event loop conflicts
        # This prevents "this event loop is already running" error when using sglang-engine
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as executor:
            middle_json, infer_result = await loop.run_in_executor(
                executor,
                _vlm_doc_analyze_wrapper,
                pdf_bytes,
                image_writer,
                backend,
                server_url,
