...
async def html_parse_main(
    resource_path: str = "",
    save_parsed_content: bool = False,
    output_dir: str = "",
):
    """
    Parse HTML files and convert to Markdown format using Docling.

    This parser handles HTML files and converts them to Markdown while preserving:
    - Text formatting and structure
    - Links and references
    - Tables and lists
    - Images (when save_parsed_content is True)
    - HTML metadata
    - CSS styling where possible

    Args:
        html_file_path: Path to the HTML file to be parsed
    ...
    """
    local_html_path = await process_resource_path(
        resource_path=resource_path,
        output_dir=output_dir if save_parsed_content else None,
    )

    file_path = Path(local_html_path)
    ...
    if save_parsed_content:
        ...
        conv_res = converter.convert(local_html_path)

    markdown_content = conv_res.document.export_to_markdown()
    logger.info("HTML file parsed successfully, preparing to return result")
