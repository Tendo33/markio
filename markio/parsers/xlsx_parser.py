...
async def xlsx_parse_main(
    resource_path: str = "",
    save_parsed_content: bool = False,
    output_dir: str = "",
):
    """
    Parse Microsoft Excel XLSX files and convert to Markdown format using Docling.

    This parser handles XLSX files and converts them to Markdown while preserving:
    ...
    Args:
        xlsx_file_path: Path to the XLSX file to be parsed
    ...
    """
    local_xlsx_path = await process_resource_path(
        resource_path=resource_path,
        output_dir=output_dir if save_parsed_content else None,
    )

    file_path = Path(local_xlsx_path)
    file_name = file_path.stem

    logger.info("Starting XLSX parsing process")
    ...
    if save_parsed_content:
        ...
        conv_res = converter.convert(local_xlsx_path)
