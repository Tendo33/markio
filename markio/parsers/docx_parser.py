...
async def docx_parse_main(
    resource_path: str = "",
    save_parsed_content: bool = False,
    output_dir: str = "",
):
    """
    Parse Microsoft Word DOCX files and convert to Markdown format using Docling.

    This parser handles DOCX files and converts them to Markdown while preserving:
    ...
    Args:
        docx_file_path: Path to the DOCX file to be parsed
    ...
    """
    local_docx_path = await process_resource_path(
        resource_path=resource_path,
        output_dir=output_dir if save_parsed_content else None,
    )

    file_path = Path(local_docx_path)
    ...
    if save_parsed_content:
        ...
        doc_converter = DocumentConverter(
            format_options={
                InputFormat.DOCX: WordFormatOption(
                    pipeline_cls=SimplePipeline, pipeline_options=pipeline_options
                )
            }
        )
        ...
        conv_res = converter.convert(local_docx_path)

    markdown_content = conv_res.document.export_to_markdown()
    logger.info("DOCX file parsed successfully, preparing to return result")
